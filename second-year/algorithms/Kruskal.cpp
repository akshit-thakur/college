...
{ /*builds the initial heap of edges*/
	for (int i = (n / 2); i > 0; i--)
...
{ //checks if edge heap is empty
	return n == 0;
}

bool duplicate(int i, int j, int n)
{ //checks for duplicates prevelant in unweighted graph
	for (int x = 0; x < n; x++)
	{
...
int main()
{
	...
	for (i = 0; i < n; i++)
	{
		for (j = 0; j < n; j++)
		{
			...
			if (i != j)
			{
				if (!duplicate(i + 1, j + 1, k) && cost[i][j] != INFY)
