...
void activity(int **&sf, int n)
{
	...
	while (j < n)
	{ //till all n activities are checked
		for (i = j + 1; i < n; i++)
		{ //start from the one next to last added activity to solution
			if (sf[0][i] >= sf[1][j])
			{ //if the two activities are compatible then add activity i to solution
				solution[c++] = sf[2][i];
