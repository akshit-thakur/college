...
void dijkastra(int v, int n)
{
	...
	for (i = 1; i < n - 1; i++)
	{ //choose w such that flag[w]=false and dist[w] is minimum;
		w = choose(flag, n);
		flag[w] = true;
		for (u = 0; flag[u] == false || u < n; u++)
		{ //all neighbours u of w such that flag[u]=false
			if (dist[u] > dist[w] + cost[w][u])
			{ //if distance through w (i.e. v -> w -> u) is less than that of path between u and v ( v -> u )
				dist[u] = dist[w] + cost[w][u];
				path[u][w] = 1; //the edge from w to u exists in the minimum distance graph/tree
				for (j = 0; j < w; j++)
				{						 //check for preceding vertices
